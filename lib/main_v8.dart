...
class _DashboardFooter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Opacity(
      opacity: 0.8,
      child: Text(
        'Risk is computed from moisture level, drying trend, and temperature. '
                    ...
                    children: [
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('WATER_NOW', '💧 Which farm needs irrigation today, and which one should I water first?'),
                        icon: const Icon(Icons.water_drop_outlined),
                        label: const Text('Irrigation today'),
                      ),
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('RISK_CHECK', '⚠️ What is the biggest risk in my farms right now?'),
                      ...
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('WATER_SCHEDULE', '🗓️ How should I water each farm over the next 3 days?'),
                        icon: const Icon(Icons.calendar_month_outlined),
                        label: const Text('3-day water plan'),
                      ),
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('COMPARE_FARMS', '📊 Which farm needs the most attention right now, and why?'),
...
ElevatedButton.icon(
  onPressed: _isSending
      ? null
      : () => _sendPrompt('TEMP_CHECK', '🌡️ Is heat stressing my crops today, and what should I do first?'),
  icon: const Icon(Icons.thermostat_outlined),
  label: const Text('Heat check'),
),
                    ],
                  ),
                ],

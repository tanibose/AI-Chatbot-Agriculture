...
class _DashboardPageState extends State<DashboardPage> {
  ...
  final Map<String, List<Map<String, dynamic>>> _seriesByDevice = {};
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      for (int i = 0; i < ids.length; i++) {
        ...
        _seriesByDevice[deviceId] = readings;
...
double _riskScore(double? latestMoist, double? latestTemp, double? dryingRatePerHr) {
  ...
  @override
  Widget build(BuildContext context) {
    const group1Order = ['Farm-1', 'Farm-2', 'Farm 3'];
    final group1Metrics = _metrics
        .where((m) => group1Order.contains(m.friendlyName))
        .toList()
      ..sort((a, b) => group1Order.indexOf(a.friendlyName)
          .compareTo(group1Order.indexOf(b.friendlyName)));
    final farm4Series = (_seriesByDevice['farm4_dataset'] ?? []).reversed.toList();
    final farm5Series = _seriesByDevice['farm5_dataset'] ?? [];
                  ...
                  : Scrollbar(
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          _Group1BarChartCard(metrics: group1Metrics),
                          const SizedBox(height: 12),
                          _Farm4LineChartCard(series: farm4Series),
                          const SizedBox(height: 12),
                          _Farm5SoilChartCard(series: farm5Series),
                          const SizedBox(height: 12),
...
class _Group1BarChartCard extends StatelessWidget {
  final List<_DeviceMetrics> metrics;
  const _Group1BarChartCard({Key? key, required this.metrics}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final moistureColor = const Color(0xFF4F46E5);
    final tempColor = const Color(0xFFF59E0B);

    final maxMoisture = metrics
        .map((m) => (m.latestMoisture ?? 0).clamp(0, 100).toDouble())
        .fold<double>(10, (a, b) => max(a, b));
    final maxTemp = metrics
        .map((m) => (m.latestTemp ?? 0).clamp(0, 100).toDouble())
        .fold<double>(10, (a, b) => max(a, b));

    final moistureTop = max(40.0, maxMoisture + 10);
    final tempTop = max(40.0, maxTemp + 5);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text(
              'Immediate field comparison for Farm-1, Farm-2, and Farm 3.',
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _legendDot(moistureColor, 'Moisture (%)'),
                _legendDot(tempColor, 'Temperature (°C)'),
              ],
            ),
            const SizedBox(height: 16),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: moistureColor.withOpacity(0.06),
                borderRadius: BorderRadius.circular(18),
                border: Border.all(color: moistureColor.withOpacity(0.12)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                    ...
                    child: BarChart(
                      BarChartData(
                        ...
                        barGroups: List.generate(metrics.length, (i) {
                          final y = (metrics[i].latestMoisture ?? 0).clamp(0, 100).toDouble();
                            ...
                            barRods: [
                              BarChartRodData(
                                ...
                                backDrawRodData: BackgroundBarChartRodData(
                                  show: true,
                                  toY: moistureTop,
                                  color: moistureColor.withOpacity(0.08),
                                ),
                              ),
                            ],
                          );
            ...
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: tempColor.withOpacity(0.08),
                borderRadius: BorderRadius.circular(18),
                border: Border.all(color: tempColor.withOpacity(0.16)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  SizedBox(
                    ...
                    child: BarChart(
                      BarChartData(
                        ...
                        barGroups: List.generate(metrics.length, (i) {
                          final y = (metrics[i].latestTemp ?? 0).clamp(0, 100).toDouble();
                          return BarChartGroupData(
                            x: i,
                            barRods: [
                              BarChartRodData(
...
class _Farm4LineChartCard extends StatelessWidget {
  final List<Map<String, dynamic>> series;
  const _Farm4LineChartCard({Key? key, required this.series}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final fillColor = const Color(0xFF8B5CF6);

    final points = <FlSpot>[];
    for (int i = 0; i < series.length; i++) {
      final y = _toDouble(series[i]['max_temp_f']);
      if (y != null) points.add(FlSpot(i.toDouble(), y));
    }
    if (points.isEmpty) return const SizedBox.shrink();

    final minY = points.map((p) => p.y).reduce(min) - 5;
    final maxY = points.map((p) => p.y).reduce(max) + 5;
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'Group 2 Analytics · Farm 4 Climate Trend',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 6),
            Text(
              'Styled temperature trend view for climate analytics.',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 14),
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [
                    ...
                    fillColor.withOpacity(0.04),
                  ],
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight,
                ),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: lineColor.withOpacity(0.14)),
              ),
              child: SizedBox(
                height: 240,
                child: LineChart(
                    ...
                    titlesData: FlTitlesData(
                      ...
                      bottomTitles: AxisTitles(
                        sideTitles: SideTitles(
                          showTitles: true,
                          reservedSize: 28,
                          interval: max(1, (series.length / 4).ceil()).toDouble(),
                          getTitlesWidget: (value, meta) {
                            final idx = value.toInt();
                            if (idx < 0 || idx >= series.length) return const SizedBox.shrink();
                            final raw = series[idx]['timestamp']?.toString() ?? '';
...
class _Farm5SoilChartCard extends StatelessWidget {
  final List<Map<String, dynamic>> series;
  const _Farm5SoilChartCard({Key? key, required this.series}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    final row = series.first;
    final values = [
      _toDouble(row['soil_ph']) ?? 0,
      _toDouble(row['organic_carbon']) ?? 0,
      _toDouble(row['nitrogen']) ?? 0,
    ];
    const labels = ['pH', 'Carbon', 'Nitrogen'];
    final colors = [
      const Color(0xFF0EA5E9),
      const Color(0xFF10B981),
      const Color(0xFFF59E0B),
    ];
    final maxY = values.fold<double>(1, (a, b) => max(a, b)) + 2;
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'Group 2 Analytics · Farm 5 Soil Profile',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 6),
            Text(
              'Styled soil indicators for profile-level interpretation.',
              style: Theme.of(context).textTheme.bodySmall,
            ),
            const SizedBox(height: 10),
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _legendDot(colors[0], 'pH'),
                _legendDot(colors[1], 'Organic carbon'),
                _legendDot(colors[2], 'Nitrogen'),
              ],
            ),
            const SizedBox(height: 14),
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: colors[1].withOpacity(0.14)),
              ),
              child: SizedBox(
                height: 240,
                child: BarChart(

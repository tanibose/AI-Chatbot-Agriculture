...
class _DevicesPageState extends State<DevicesPage> {
  ...
  Future<Map<String, String>> getDeviceNames() async {
    ...
    // ✅ Always include Farm 3 (reference dataset) without requiring registration.
  ...
  Future<void> _deleteDevice(String deviceId) async {
    // Prevent deleting the built-in reference farm.
    if (deviceId == "farm3_dataset") return;
...
class _FetchDataPageState extends State<FetchDataPage> {
  ...
  Future<void> _loadInitial() async {
    ...
    // Farm 3 is a reference dataset (not stored in Amplify/AppSync).
  ...
  Widget build(BuildContext context) {
    ...
    final pageButtons = List<Widget>.generate(maxPage, (i) {
    ...
    if (nextToken != null) {
      pageButtons.add(
        Padding(
          ...
          child: OutlinedButton(
            onPressed: () async {
              await _ensurePageLoaded(maxPage + 1);
            },
            child: const Text('Next ›'),
          ),
        ),
                        ...
                        child: ScrollConfiguration(
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final temp =
                                  item['temperature']?.toString() ?? 'N/A';
                              final moisture =
                                  item['moisture']?.toString() ?? 'N/A';
                              final tsRaw =
...
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
// DASHBOARD PAGE (Option 2): Metrics + Confidence across devices
// -----------------------------------------------------------------------------
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  // ✅ Farm 3 is a built-in dataset served by the FastAPI backend (CSV-backed),
  // so we should NOT query Amplify/AppSync for it.
  if (deviceId == "farm3_dataset") {
    try {
      final uri = Uri.parse(farm3ReadingsEndpoint).replace(
      ...
      final readingsRaw = (decoded["readings"] as List<dynamic>? ?? []);

      return readingsRaw
          .map((e) => Map<String, dynamic>.from(e as Map))
          .toList();
    } catch (e) {
      debugPrint("[Farm3] Failed to fetch /farm3/readings: $e");
      return [];
    }
  }
  ...
  String _riskFromMoisture(double? m) {
    if (m == null) return 'WATCH';
    if (m < 35) return 'HIGH';
    if (m < 50) return 'WATCH';
    return 'GOOD';
  }

  // Simple confidence based on data quality (0..1)
  double _confidenceScore(List<Map<String, dynamic>> readings) {
    if (readings.isEmpty) return 0.2;

    // recency
    final latestTs = DateTime.tryParse(readings.first['timestamp']?.toString() ?? '');
    double recency = 0.5;
    if (latestTs != null) {
      final ageMin = DateTime.now().difference(latestTs).inMinutes.abs();
      if (ageMin <= 30) {
        recency = 1.0;
      } else if (ageMin <= 180) recency = 0.8;
      else if (ageMin <= 720) recency = 0.6;
      else recency = 0.4;
    }
    ...
    for (final r in readings) {
      if (r['moisture'] != null) haveMoist++;
      if (r['temperature'] != null) haveTemp++;
    }
    final completeness = ((haveMoist / readings.length) + (haveTemp / readings.length)) / 2.0;

    // regularity: check median gap
    double regularity = 0.7;
    final times = readings
        .map((r) => DateTime.tryParse(r['timestamp']?.toString() ?? ''))
        .whereType<DateTime>()
        .toList();
    if (times.length >= 3) {
      ...
      for (int i = 0; i < times.length - 1; i++) {
        gaps.add(times[i].difference(times[i + 1]).inMinutes.abs());
      }
      gaps.sort();
      final median = gaps[gaps.length ~/ 2];
      if (median <= 20) {
        regularity = 1.0;
      } else if (median <= 60) regularity = 0.85;
      else if (median <= 180) regularity = 0.65;
      else regularity = 0.45;
    }

    final score = (0.45 * recency) + (0.35 * completeness) + (0.20 * regularity);
  ...
  _DeviceMetrics _computeMetrics({
    ...
    final risk = _riskFromMoisture(latestMoist);
    final conf = _confidenceScore(readings);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Using last $perDeviceLimit readings per device • Trend window: last $hours hours'),
            const SizedBox(height: 8),
            const Text(
              'Tip: Use Global Farm Advisor to ask “Which farm is driest?” or “Compare all farms today.”',
...
class _DashboardFooter extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Opacity(
      opacity: 0.8,
      child: Text(
        'Confidence is estimated from data recency, completeness, and timestamp regularity. '
        'It is not a medical or agronomy diagnosis.',
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: _ConfidenceTile(confidence: metrics.confidence),
                ),
              ],
            ),
          ],
...
class _ConfidenceTile extends StatelessWidget {
  ...
  const _ConfidenceTile({Key? key, required this.confidence}) : super(key: key);
...
class _GlobalFarmAdvisorPageState extends State<GlobalFarmAdvisorPage> {
  ...
  void initState() {
    ...
    _selectedDeviceIds.addAll(widget.devices.keys);
    ...
    _messages.add(
      _ChatMessage(
        ...
        text:
            ...
            "I can compare multiple farms and give practical advice.\n\n"
            "Select one or more devices above, then ask your question.",
      ),
    );
  }
  ...
  Future<void> _sendPrompt(String promptId, String userLabel) async {
    ...
    try {
      ...
      for (int i = 0; i < selected.length; i++) {
        farmLabels[selected[i]] = 'Farm ${i + 1}';
      }
                    ...
                    children: [
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('WATER_NOW', '💧 Which farm needs water first?'),
                        icon: const Icon(Icons.water_drop_outlined),
                        label: const Text('Water first'),
                      ),
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('RISK_CHECK', '⚠️ Any risk or problem now?'),
                      ...
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('WATER_SCHEDULE', '🗓️ When should I water next?'),
                      ...
                      ElevatedButton.icon(
                        onPressed: _isSending
                            ? null
                            : () => _sendPrompt('COMPARE_FARMS', '📊 Compare farms (best vs worst)'),
                        icon: const Icon(Icons.query_stats_outlined),
                        label: const Text('Compare farms'),
                      ),

ElevatedButton.icon(
  onPressed: _isSending
      ? null
      : () => _sendPrompt('TEMP_CHECK', '🌡️ Is temperature affecting my farms?'),
  icon: const Icon(Icons.thermostat_outlined),
  label: const Text('Temp check'),
),
                    ],
                  ),
                ],
